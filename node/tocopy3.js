...
app.get('/showInterest', function (req,res) {
    client.getQuotes({symbol: ['BTC,ETH,LINK,CRO,SXP,MATIC,RSR,DOT,VET,BLZ,ADA'], convert: 'EUR'}).then((prices) => {
        db.query("SELECT * FROM interest WHERE MONTH(date) = MONTH(CURRENT_DATE)", function(err,result) {
            if(err) throw err;
            var sum = 0;
            for(i=0;i<result.length;i++) {
                if(result[0].coin == 'BTC')
                    sum += result[0].amount * prices.data.BTC.quote.EUR.price;
                else if(result[0].coin == 'CRO')
                    sum += result[0].amount * prices.data.CRO.quote.EUR.price;
                else if(result[0].coin == 'ETH')
                    sum += result[0].amount * prices.data.ETH.quote.EUR.price;
                else if(result[0].coin == 'LINK')
                    sum += result[0].amount * prices.data.LINK.quote.EUR.price;
                res.write(result[i].coin+" - ");
                res.write(result[i].amount+" - ");
