            ...
            CoinGeckoClient.coins.markets({per_page:[250],page:[2]}).then((markets2) => {
                ...
                CoinGeckoClient.simple.price({ids: geckoIds,vs_currencies: ['eur','usd'],}).then((prices) => {
                    db.query(sqlHoldings, function(err,result) {
                        ...
                        res.write("<table><tr><td><b>Coin</b></td><td>Deposits</td><td>Deposits (%)</td><td>Holdings</td><td>Gecko Price</td><td>Portfolio (%)</td><td>Profit</td></tr>");
                        for(i=0;i<geckoIds.length;i++) {
                            ...
                            if(!stablecoins.includes(coinName)) {
                                ...
                                res.write("<td>"+(myround(deposits[i])).toString()+"</td>");
                                res.write("<td>"+myround(percentage)+"</td>");
                                res.write("<td>"+holdings[i].toString()+"</td>");
                                res.write("<td>"+geckoprice.toString()+"</td>");
                                res.write("<td>"+myround(percentage2)+"</td>");
                                res.write("<td>"+(myround(tempValue - deposits[i])).toString()+"</td></tr>");
