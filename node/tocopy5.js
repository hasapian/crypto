	...
	console.log("The enum: " + CoinsEnum[result[i].coin]);
        ...
        if(result[i].coin == 'USDT') {
            ...
            if(result[i].coin == 'BTC')
                ...
                coinIndex = 3;
            else if(result[i].coin == 'SXP')
                coinIndex = 4;
            else if(result[i].coin == 'MATIC')
                coinIndex = 5;
            else if(result[i].coin == 'RSR')
                coinIndex = 6;
            else if(result[i].coin == 'VET')
                coinIndex = 7;
            else if(result[i].coin == 'BLZ')
                coinIndex = 8;
            else if(result[i].coin == 'DOT')
                coinIndex = 9;
            else if(result[i].coin == 'ADA')
                coinIndex = 10;
            else if(result[i].coin == 'CEL')
                coinIndex = 11;
            switch(x) {
                case 1:
                    deposits[coinIndex]+=amount;
                    break;
                case 2:
                    posessions[coinIndex]+=amount;
                    break;
                case 3:
                    trades[coinIndex]+=amount;
                    break;
                default:
                    //should do nothing
            }
